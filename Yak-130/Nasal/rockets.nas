...
#ammo counter
props.globals.initNode("/controls/armament/rocketsLeft", 25, "INT");
props.globals.initNode("/controls/armament/rocketsCount", 25, "DOUBLE");
var reload = func {
	if( getprop("/gear/gear[0]/wow") and getprop("/gear/gear[1]/wow") and getprop("/gear/gear[2]/wow") and (getprop("/velocities/groundspeed-kt") < 2) ) {
		setprop("/controls/armament/rocketsLeft", 25);
		setprop("/controls/armament/rocketsCount", 25);
		screen.log.write("S-13 rockets reloaded (25 rockets per pod)", 1, 0.6, 0.1);
	}
	else {
	...
	if(triggerState and getprop("/controls/armament/rocketsLeft") > 0) {
		...
		if(mounted2L or mounted6R) {
			...
			interpolate("/controls/armament/rocketsCount", 0, 
				fireTime*(rocketsLeft/25));
			outOfAmmo.restart(fireTime*(rocketsLeft/15));
		}
	}
	else {
		setprop("/controls/armament/trigger-S-13-L", 0);
