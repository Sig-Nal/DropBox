...
#ammo counter
props.globals.initNode("/controls/armament/roundsLeft", 1200, "INT");
props.globals.initNode("/controls/armament/roundsCount", 1200, "DOUBLE");
var reload = func {
	if( getprop("/gear/gear[0]/wow") and getprop("/gear/gear[1]/wow") and getprop("/gear/gear[2]/wow") and (getprop("/velocities/groundspeed-kt") < 2) ) {
		setprop("/controls/armament/roundsLeft", 1200);
		setprop("/controls/armament/roundsCount", 1200);
		screen.log.write("GSh-23 cannon reloaded (1200 rounds)", 1, 0.6, 0.1);
	}
	else {
	...
	if(triggerState and getprop("/controls/armament/roundsLeft") > 0) {
		...
		if(mounted) {
			...
			interpolate("/controls/armament/roundsCount", 0, 
				fireTime*(roundsLeft/1200));
			outOfAmmo.restart(fireTime*(roundsLeft/1200));
		}
	}
	else {
		setprop("/controls/armament/trigger-GSh-23", 0);
